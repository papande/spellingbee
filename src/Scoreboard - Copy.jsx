...
const Scoreboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeState = onValue(stateRef, (snapshot) => snapshot.exists() && setGameState(snapshot.val()));

    const playersRef = ref(db, 'players');
    const unsubscribePlayers = onValue(playersRef, (snapshot) => snapshot.exists() && setPlayers(snapshot.val()));

    return () => { unsubscribeState(); unsubscribePlayers(); };
  }, []);

  const activePlayerName = players[gameState.activePlayerId]?.name || "Waiting...";
  ...
  // 5. RENDER UI
  if (gameState.winnerId && players[gameState.winnerId]) {
    ...
    return (
      <div style={{ ...styles.wrapper, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: '#000' }}>
        ...
        <div style={{ fontSize: '1.5rem', color: '#888' }}>
          Spelled {winner.wordHistory.filter(w => w.correct).length} words correctly!
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={styles.wrapper}>
      {/* ... your existing scoreboard HTML goes exactly here ... */}
